  ...
  return (
    <section className="py-32 relative overflow-hidden bg-[color:var(--glass-bg)] bg-dots-pattern" id="testimonials">
      ...
      <div className="relative w-full flex overflow-x-hidden py-20 z-10 group" dir="ltr">
        ...
        <div className="flex animate-[marquee_80s_linear_infinite] group-hover:[animation-play-state:paused] whitespace-nowrap items-center shrink-0 w-max">
          {[...basePartners, ...basePartners, ...basePartners, ...basePartners, ...basePartners].map((partner, i) => (
            ...
            >
              ...
              <div className="absolute -top-14 left-1/2 -translate-x-1/2 opacity-0 group-hover/partner:opacity-100 transition-all duration-300 pointer-events-none z-50 transform group-hover/partner:-translate-y-2 w-max max-w-[250px]">
                <div className="bg-black/90 backdrop-blur text-white px-4 py-2 rounded-xl text-sm font-bold border border-white/10 shadow-[0_0_15px_var(--partner-color)] text-center break-words">
                  {partner.name}
                </div>
              </div>
              ...
              >
                ...
                <div className="relative z-10 w-full h-full p-4 flex items-center justify-center">
                  {partner.imageUrl ? (
                    ...
                    <h4 className="text-lg md:text-xl font-bold text-white uppercase drop-shadow-md text-center px-2 break-words leading-tight w-full">{partner.name}</h4>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="flex animate-[marquee_80s_linear_infinite] group-hover:[animation-play-state:paused] whitespace-nowrap items-center shrink-0 w-max" aria-hidden="true">
          {[...basePartners, ...basePartners, ...basePartners, ...basePartners, ...basePartners].map((partner, i) => (
            <div 
      ...
      <div className="max-w-7xl mx-auto px-6 mt-32 relative z-10">
         <div className="grid md:grid-cols-3 gap-8">
            {[
              { num: '+300%', title: 'زيادة في المبيعات', desc: 'لأحد عملائنا في قطاع التجزئة بفضل إعادة بناء رحلة العميل بالكامل خلال 3 أشهر فقط.' },
              { num: '+8x', title: 'مضاعفة ROAS', desc: 'تقليل تكلفة الاستحواذ على العميل CAC مع مضاعفة العائد الإعلاني.' },
              { num: '-40%', title: 'تقليل المرتجعات', desc: 'بفضل التحليل الدقيق للسلوك وتوضيح وصف المنتجات بدقة وتقنية النماذج الذكية.' }
            ].map((stat, i) => (
              <motion.div 
                key={i}
                ...
                className="p-10 rounded-3xl text-center relative overflow-hidden group hover:-translate-y-2 transition-all duration-500 bg-black/40 border border-white/5 hover:border-[color:var(--color-brand-blue-val)] hover:shadow-[inset_0_4px_15px_rgba(0,0,0,1),0_0_20px_rgba(79,142,247,0.3)] shadow-[inset_0_4px_15px_rgba(0,0,0,1),0_0_20px_rgba(0,0,0,0.5)]"
              >
                ...
                <div className="absolute top-0 right-0 w-32 h-32 bg-[color:var(--color-brand-blue-val)] opacity-5 blur-[40px] rounded-full group-hover:opacity-10 transition-opacity" />
                ...
                <h3 className="text-2xl font-bold mb-4 text-[color:var(--color-text-main)] group-hover:text-[color:var(--color-brand-blue-val)] transition-colors duration-300">{stat.title}</h3>
