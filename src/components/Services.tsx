  ...
  return (
    <section className="py-32 relative z-10 bg-radial-glow" id="services" ref={sectionRef}>
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-3 max-md:gap-2 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-4 w-full px-2">
          {services.map((service, index) => {
            ...
            return (
              <motion.div
                ...
                className={isMobile ? "flex flex-col h-full" : "snap-center shrink-0 w-[85vw] md:w-auto flex-1 flex flex-col h-full"}
              >
                {isMobile ? (
                  <motion.div 
                    initial={{ opacity: 0, scale: 0.9 }}
                    ...
                    className="active:scale-95 transition-all duration-300 rounded-2xl overflow-hidden group relative flex flex-col items-center justify-center p-3 text-center shadow-[inset_0_2px_6px_rgba(0,0,0,0.8),0_1px_1px_rgba(255,255,255,0.05)] border border-white/10 bg-black/40 w-full cursor-pointer h-full py-4 px-1"
                  >
                    <div className="absolute inset-0 bg-gradient-to-br from-[color:var(--color-brand-blue-val)] to-[color:var(--color-brand-purple-val)] opacity-0 group-hover:opacity-10 transition-opacity duration-300 pointer-events-none z-0" />
                    <div className="relative z-10 flex flex-col items-center justify-center h-full w-full">
                      <div className="p-2 rounded-lg bg-black/50 border border-white/5 text-[color:var(--color-brand-blue-val)] flex items-center justify-center mb-2.5">
                        {React.cloneElement(serviceIcon, { className: "w-5 h-5" })}
                      </div>
                      <h3 className="text-[10px] font-bold text-white group-hover:text-[color:var(--color-brand-blue-val)] transition-colors duration-300 leading-tight">
                        {service.title}
                      </h3>
                    </div>
                  </motion.div>
                 ...
                 {isStoreService && (
                   isMobile ? (
                     <div className="flex flex-col gap-4">
                       <h4 className="text-sm font-bold mb-2 text-[color:var(--color-brand-green-val)] uppercase tracking-wider flex items-center gap-2">
                         <div className="w-2 h-2 rounded-full bg-[color:var(--color-brand-green-val)] shadow-[0_0_8px_var(--color-brand-green-val)]" />
                       ...
                       <div className="mt-2">
                         <button 
                           onClick={() => { setOpenId(null); triggerBookingModal('إنشاء المتاجر الإلكترونية'); }}
                           className="w-full py-4 rounded-xl font-bold bg-gradient-to-r from-brand-blue to-brand-purple text-white border-none hover:shadow-[0_0_20px_rgba(124,58,237,0.5)] transition-all cursor-pointer"
                         >
                           ابدأ إنشاء متجرك الآن
                         </button>
                       </div>
                     </div>
                   ) : (
